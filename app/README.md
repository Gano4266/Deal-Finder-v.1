...
Copy `.env.example` to `.env.local` for local-only settings. The public report
form submits in-app through `/api/reports`. Local/dev accepts and logs test
submissions when HubSpot is not configured; production returns an honest
unavailable message unless HubSpot or a fallback webhook is configured. For local fixture testing only,
`DEAL_FINDER_OPERATING_DATE=2026-06-02` can pin the operating date; production
